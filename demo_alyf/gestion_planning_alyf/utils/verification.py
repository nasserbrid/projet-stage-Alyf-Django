...
def verifie_si_planning__change(instructor):
   ...
   # print(formateur)  
    ...
    # Convertir les fichiers Excel en CSV pour une comparaison plus fiable
     pd.read_excel(fileA, sheet_name='DEV WEB', engine='openpyxl').to_csv('fileA.csv', index=False)
     pd.read_excel(fileB, sheet_name='DEV WEB', engine='openpyxl').to_csv('fileB.csv', index=False)

     print("CSV files created for comparison")
    ...
    if os.path.exists('fileA.csv'):
        os.remove('fileA.csv')
    if os.path.exists('fileB.csv'):
        os.remove('fileB.csv')
